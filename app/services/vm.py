...
def createVm(vmName,osType,cpuCore,ram,vRam,graphicsController,network,storageType,storageSize,storageName,storageBootable,osImageName):
    ...
    os.system('vboxmanage createhd --filename C:\\Users\\Gabichus\\Desktop\\Disertatie\\hd\\{storageName} --size {storageSize} --variant {storageType}'.format(storageName=storageName,storageSize=storageSize,storageType=storageType))
    ...
    os.system('vboxmanage storageattach {vmName} --storagectl "{storageName}" --port 0 --device 0 --type hdd --medium C:\\Users\\Gabichus\\Desktop\\Disertatie\hd\{storageName}.vdi'.format(vmName=vmName,storageName=storageName))
